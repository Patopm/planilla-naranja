		...
		for (let row = 0; row < rows; row++) {
			for (let col = 0; col < cols; col++) {
				dots.push({
					...
					baseOpacity: 0.68 + Math.random() * 0.16,
				});
			}
		}
	...
	const draw = useCallback(() => {
		...
		if (hasMouse) {
			...
			ambientGlow.addColorStop(
				0,
				`rgba(${glowRgb.r}, ${glowRgb.g}, ${glowRgb.b}, 0.045)`,
			);
			ambientGlow.addColorStop(
				0.18,
				`rgba(${glowRgb.r}, ${glowRgb.g}, ${glowRgb.b}, 0.028)`,
			);
			ambientGlow.addColorStop(
				0.45,
				`rgba(${glowRgb.r}, ${glowRgb.g}, ${glowRgb.b}, 0.012)`,
			);
			ambientGlow.addColorStop(
				1,
			...
			coreGlow.addColorStop(
				0,
				`rgba(${glowRgb.r}, ${glowRgb.g}, ${glowRgb.b}, 0.09)`,
			);
			coreGlow.addColorStop(
				0.5,
				`rgba(${glowRgb.r}, ${glowRgb.g}, ${glowRgb.b}, 0.03)`,
			);
			coreGlow.addColorStop(
				1,
				`rgba(${glowRgb.r}, ${glowRgb.g}, ${glowRgb.b}, 0)`,
		...
		for (const dot of dotsRef.current) {
			...
			const waveOpacity = Math.min(0.98, dot.baseOpacity + wave * 0.08);
			...
			if (distSq < proxSq) {
				...
				opacity = Math.min(1, waveOpacity + easedT * 0.6);
				scale = waveScale + easedT * 0.82;
			...
			if (glow > 0) {
				...
				gradient.addColorStop(
					0,
					`rgba(${glowRgb.r}, ${glowRgb.g}, ${glowRgb.b}, ${glow * 0.52})`,
				);
				gradient.addColorStop(
					0.35,
					`rgba(${glowRgb.r}, ${glowRgb.g}, ${glowRgb.b}, ${glow * 0.18})`,
				);
				gradient.addColorStop(
					1,
					`rgba(${glowRgb.r}, ${glowRgb.g}, ${glowRgb.b}, 0)`,
				...
				ctx.beginPath();
				ctx.arc(dot.x, dot.y, radius * 3.8, 0, Math.PI * 2);
